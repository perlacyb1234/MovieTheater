...
public class Main {
    ...
    public void test_trade_pay(AlipayTradeService service) {
        ...
        String sellerId = "2088102177923954";
    ...
    // 测试当面付2.0生成支付二维码
    public String test_trade_precreate(OrderVo orderVo) {
        // (必填) 商户网站订单系统中唯一订单号，64个字符以内，只能包含字母、数字、下划线，
        // 需保证商户系统端不能重复，建议通过数据库sequence生成，
        String outTradeNo = "tradeprecreate" + orderVo.getOrderId();

        // (必填) 订单标题，粗略描述用户的支付目的。如“xxx品牌xxx门店当面付扫码消费”
        String subject = "狗眼电影在线购票扫码消费";
        ...
        String body = orderVo.getSeatsName();

        // 商户操作员编号，添加此参数可以为商户操作员做销售统计
        String operatorId = "Cyb";
        ...
        //GoodsDetail goods1 = GoodsDetail.newInstance(orderVo.getOrderId(), orderVo.getCinemaName() + "电影票" + orderVo.getSeatsName(), 1231312331, 1);
        // 创建好一个商品后添加至商品明细列表
        goodsDetailList.add(goods1);
        ...
        switch (result.getTradeStatus()) {
            case SUCCESS:
                ...
                // 需要修改为运行机器上的路径
                String filePath = String.format("D:\\JavaProject\\MovieTheater\\guns-pay\\src\\main\\resources\\static\\erweima\\qr-%s.png",
